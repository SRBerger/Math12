...
A bar plot is often more effective than a pie chart for displaying categorical data, especially when dealing with a large number of categories. To create a bar plot, we can use the $\txt{barplot()}$ function. Note that the function accepts the $\txt{table}$ object that we created earlier.
...
```{webr-r}
## Use Freq=FALSE to get a histogram based on the relative frequency. .
...
iv) Create a conditional frequency barplot for $\txt{Browser}$ with respect to $\txt{Cat\_Dog}$. Discuss the relationship between the two categorical variables.
...
**Click [HERE](https://coursesite.lehigh.edu/mod/quiz/view.php?id=6230325){target="_blank"} to submit your answers.**
