...
2.  Let us recall the formulas for the sample variance.
...
i) Some very enthusiastic Disney fans have collected the following data on average wait times per year for the ride \textit{Soarin' Around the World} at Epcot.  

```{webr-r}
Year = 2014:2025
Wait.Time = c(40,60,51,48,42,40,33,34,23,25,24,20)
Disney.Data = data.frame(Year,Wait.Time)
print(Disney.Data)
```

ii) For the data given in part i, compute the mean and standard deviation of average wait times for this ride.
```{webr-r}

```

iii) Make a time series plot of average wait times for \textit{Soarin' Around the World} between 2014 and 2025.  Be sure to include a title and labels for the x and y-axes.  
```{webr-r}

```


**Click [HERE](https://coursesite.lehigh.edu/mod/quiz/view.php?id=6207408){target="_blank"} to submit your answers.**